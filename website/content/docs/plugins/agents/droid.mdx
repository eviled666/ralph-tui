...
Factory Droid supports passing a model name and reasoning effort level. Configure via `agentOptions`:
...
Key behaviors:
1. **Non-interactive mode**: `--print` and `-p` pass the prompt
2. **Tracing**: `--output-format stream-json` when tracing enabled
